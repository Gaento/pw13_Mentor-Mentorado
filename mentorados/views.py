...
def mentorados(request):
    ...
    if request.method == 'GET':
        ...
        estagio_flat = [i[1] for i in Mentorados.estagio_choices]
        ...
        return render(request,'mentorados.html', {'estagios': Mentorados.estagio_choices, 'navigators': navigators, 'mentorados': mentorados, 'estagios_flat' : estagio_flat, 'qtd_estagios': qtd_estagios})
...
def reunioes(request):
    if request.method == 'GET':
        ...
        disponibilidades = DisponibilidadeHorarios.objects.filter(mentor=request.user).filter(
            data_inicial__gte=(data - timedelta(minutes=50)),data_inicial__lte=(data + timedelta(minutes=50))
            )
